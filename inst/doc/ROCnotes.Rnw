@
%\VignetteIndexEntry{ROC notes}
%\VignetteDepends{ROC, Biobase}
%\VignetteKeywords{ROC curves}
%\VignettePackage{ROC}
...
We confine activities to the first 50 genes in eset so that
this vignette is computable in reasonable CPU time.

<<>>=
library(Biobase)
data(eset)
myauc <- function(x) {
 dx <- cov1-1 # phenoData is installed
...
print(geneNames(eset[1:50,])[allAUC >= 
       max(allAUC,na.rm=TRUE) & !is.na(allAUC)])

print(geneNames(eset[1:50,])[allpAUC1 >= 
...
this is easy to carry out.  (We use a very small number of
resamplings (5) to get through this in a reasonable execution time.)
...
# 
#File /misc/homes/stvjc/madman/Rpacks/ROC.Rcheck/ROC/doc/ROCnotes.R :
#Error in print.default((1:50)[apply(rout, 1, mean) > 35]) :
#        can not set length of non-vector
#print((1:50)[apply(rout,1,mean)>35])
